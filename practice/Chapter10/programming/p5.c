...
void analyze_card(int num_in_ranks[],  int num_in_suits[]);
void print_class();
...
int three;
int pairs;
int big_straight_flush = 0;
int little_straight_flush = 0;
...
int main(){
	...
	while(1){
		...
		analyze_card(num_in_ranks, num_in_suits);
		print_class();
...
void analyze_card(int num_in_ranks[], int num_in_suits[]){
	...
	for(rank = 0; rank < NUM_RANKS - 1; rank++){
		if(num_in_ranks[rank] == 1 && num_in_ranks[rank + 1] == 1){
			...
			if(number == 3){
				if(rank - 2 == 0){
					if(num_in_ranks[12] == 1)
						little_straight_flush = 1;
				}
			}
		}
		else{
...
void print_class(){
	...
	if(big_straight_flush && flush){
		printf("Big straight flush.\n");
		return;
	}
	if(little_straight_flush && flush){
		printf("little straight flush.\n");
		return;
	}
